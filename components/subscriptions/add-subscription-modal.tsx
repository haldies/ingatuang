    ...
    <Modal visible={visible} animationType="slide" transparent={false} onRequestClose={onClose} statusBarTranslucent>
      <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]} edges={['top', 'bottom']}>
        ...
        <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.previewContainer}>
            <TouchableOpacity onPress={handlePickImage} activeOpacity={0.8}>
              {formData.iconType === 'image' && formData.imageUri ? (
                <View style={[styles.imageWrapper, { borderRadius: getRadius(240, 'large'), borderColor: theme.border }]}>
                  <Image source={{ uri: formData.imageUri }} style={styles.previewImage} />
                </View>
              ) : (
                <View style={[styles.iconWrapper, { backgroundColor: formData.color, borderRadius: getRadius(240, 'large'), borderColor: theme.border }]}>
                  <Ionicons name={formData.icon as any} size={48} color="#fff" />
                </View>
              )}
              <View style={[styles.cameraBadge, { backgroundColor: theme.tint, borderColor: theme.background }]}>
             ...
             <View style={styles.section}>
                ...
                <View style={styles.brandGrid}>
                   {PRESET_BRANDS.map((brand) => (
                      ...
                      >
                         <View style={[styles.brandIcon, { backgroundColor: brand.color, borderRadius: getRadius(120, 'small') }]}>
          ...
          <View style={styles.inputSection}>
            ...
            <TextInput
              style={[styles.input, { backgroundColor: theme.card, borderColor: theme.border, color: theme.text, borderRadius: getRadius(120, 'small') }]}
              value={formData.name}
          ...
          <View style={styles.row}>
            <View style={[styles.inputSection, { flex: 1 }]}>
              ...
              <TouchableOpacity
                style={[styles.pickerBtn, { backgroundColor: theme.card, borderColor: theme.border, borderRadius: getRadius(120, 'small') }]}
                onPress={() => setShowBillingCyclePicker(true)}
              >
                <Ionicons name={selectedBillingCycle?.icon as any} size={18} color={theme.textSecondary} />
                <Text style={[styles.pickerText, { color: theme.text }]}>{selectedBillingCycle?.label}</Text>
            ...
            <View style={[styles.inputSection, { flex: 1 }]}>
              ...
              >
                ...
                <Text style={[styles.pickerText, { color: theme.text }]}>{formatDate(formData.startDate)}</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={styles.inputSection}>
            ...
            <TextInput
              style={[styles.input, styles.multilineInput, { backgroundColor: theme.card, borderColor: theme.border, color: theme.text, borderRadius: getRadius(120, 'small') }]}
        ...
        <View style={[styles.footer, { borderTopColor: theme.border }]}>
          <TouchableOpacity
            style={[styles.submitBtn, { backgroundColor: theme.tint, borderRadius: getRadius(120, 'small'), opacity: loading ? 0.5 : 1 }]}
            onPress={handleSubmit}
            disabled={loading}
          >
            <Ionicons name="checkmark-circle" size={20} color="#fff" />
            <Text style={styles.submitBtnText}>{loading ? 'Menyimpan...' : subscription ? 'Perbarui' : 'Simpan'}</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
      ...
      <Modal visible={showCalendar} transparent animationType="fade">
        <Pressable style={styles.overlay} onPress={() => setShowCalendar(false)}>
          <Pressable style={[styles.sheetContent, { backgroundColor: theme.background, borderTopLeftRadius: getRadius(200, 'large'), borderTopRightRadius: getRadius(200, 'large') }]} onPress={(e) => e.stopPropagation()}>
            ...
            <Calendar
              ...
              markedDates={{
                [formData.startDate.toISOString().split('T')[0]]: { selected: true, selectedColor: theme.tint }
              }}
              theme={{
      ...
      <Modal visible={showBillingCyclePicker} transparent animationType="fade">
        <Pressable style={styles.overlay} onPress={() => setShowBillingCyclePicker(false)}>
          <Pressable style={[styles.sheetContent, { backgroundColor: theme.background, borderTopLeftRadius: getRadius(200, 'large'), borderTopRightRadius: getRadius(200, 'large') }]} onPress={(e) => e.stopPropagation()}>
            <View style={[styles.sheetHeader, { borderBottomColor: theme.border, borderBottomWidth: 1, paddingBottom: 16 }]}>
            ...
            <ScrollView style={styles.cycleList} contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
              {billingCycles.map((cycle) => (
                <TouchableOpacity
                  key={cycle.value}
                  style={[
                    styles.cycleItem,
                    { backgroundColor: theme.card, borderRadius: getRadius(120, 'small') },
                    formData.billingCycle === cycle.value && { backgroundColor: theme.tint + '10', borderColor: theme.tint, borderWidth: 1 }
                  ]}
                  onPress={() => handleBillingCycleSelect(cycle.value as any)}
                >
                  ...
                  {formData.billingCycle === cycle.value && <Ionicons name="checkmark-circle" size={24} color={theme.tint} />}
                </TouchableOpacity>
              ))}
            </ScrollView>
          </Pressable>
